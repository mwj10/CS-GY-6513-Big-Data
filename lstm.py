...
def train(ticker, look_back=60,epochs=1,batch_size=64):
    ...
    print("#### Training {ticker_name} model...".format(ticker_name=ticker)+" ####");
    ## Data Transformation
    df = getData(ticker);
    ...
    train_predict = train_predict.reshape(1,train_predict.shape[0]).tolist()[0]
...
def predict(ticker,numOfDays=5,look_back=60):
    ...
    print("#### Predicting Next {num} for {ticker} ####".format(num=numOfDays,ticker=ticker));
    print("Fetching data ...")
    # load data
    ...
    model = tf.keras.models.load_model(ticker)
