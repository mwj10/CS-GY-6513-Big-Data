...
def getPredictions(ticker,conn,date=None):
    ...
    cur = conn.cursor()
    if date==None:
        sqlCmd = "SELECT * FROM predicted_daily_prices WHERE ticker_id = {ticker_id}".format(ticker_id=ticker_id);
        return pd.read_sql(sqlCmd,con=conn)
    else:
        sqlCmd = "SELECT * FROM predicted_daily_prices WHERE ticker_id = {ticker_id} AND date={date}".format(ticker_id=ticker_id,date=date);
        cur.execute(sqlCmd);
        res = cur.fetchone();
        return res[0];
