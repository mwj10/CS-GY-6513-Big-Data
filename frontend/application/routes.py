...
def index(search=""):
    ...
    for stock in stocks:
        ...
        news[stock][0]['providerPublishTime'] = ago(datetime.datetime.fromtimestamp(news[stock][0]['providerPublishTime']))
...
def quote(name, period="1d"):
    ...
    earningsTimestampStart = datetime.datetime.fromtimestamp(tick['earningsTimestampStart'])
    tick['earningsTimestampStart'] = earningsTimestampStart.strftime("%b %d, %Y")
    earningsTimestampEnd = datetime.datetime.fromtimestamp(tick['earningsTimestampEnd'])
...
def gm(stock,period,interval,color):
    ...
    fig.add_trace(
        go.Scatter(x=df['Date-Time'], y=df["Open"],
                    fill='tozeroy', name="Value",
